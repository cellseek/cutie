...
def process_video(cfg: DictConfig):
    ...
    with torch.inference_mode():
        with torch.amp.autocast(device, enabled=(use_amp and device == "cuda")):
            ...
            for mask_name in all_mask_frames:
                ...
                mask_torch = index_numpy_to_one_hot_torch(mask_np, num_objects + 1).to(
                    device
                )
    ...
    (
        print(
            f"Max allocated memory (MB): {torch.cuda.max_memory_allocated() / (2**20)}"
        )
        if device == "cuda"
        else None
    )
...
def check_to_clear_non_permanent_cuda_memory(
    processor: InferenceCore, device, mem_cleanup_ratio
):
    if "cuda" in device:
...
if __name__ == "__main__":
    ...
    initialize(
        version_base="1.3.2", config_path="cutie/config", job_name="process_video"
    )
